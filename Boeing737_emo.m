...
%           r_sum = r_sum+r_agent;
          ...
          if fq(i) ~= 0
                swarm(i, 3, 1)= swarm(i, 3, 1)+fq(i)/8*dt;
          else
...
%         end
         ...
         wfig1 = rectangle('Position',[xdim/2.0-2*x_exit,ydim-space_above-wall_moving(iter),4*x_exit,wall_moving(iter)],'FaceColor','r');
         %wfig2 = rectangle('Position',[xdim/2.0-2*x_exit,0,4*x_exit,wall_moving(iter)],'FaceColor','r');
         %grid off
         set(gcf, 'Position', [100 100 2.5*(xdim-space_out) 5*(ydim-space_above-space_below)]);
         frame = getframe;

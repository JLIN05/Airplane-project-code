function [ p,numx,numy,xdim,ydim,x,y ]=Boeing777_plane3(rb,re1,re2,cb,ce,ab,ae,cwb,cwe,rwb,rwe,sd,cd,lrb,lre,ncb,nce,as,rs,cs,space_front,space_back,x_exit)
...
for i=(2*space_front+space_back+2*x_exit+2*rb*rwb+2*re1*rwe+1):(3*space_front+2*space_back+3*x_exit+2*rb*rwb+(2*re1+1)*rwe+lre)
    for j=1:NCe
      p(i,j) = p(i,j) + 2*rs*(chairdim_e - yce(j));
      p(i,N-j+1) = p(i,N-j+1) + 2*rs*(chairdim_e - yce(j));
    end
    for j=1:((NCe-extension)/2.0 + extension)
      p(i,(N-1)/2.0-j+1) = p(i,(N-1)/2.0-j+1) + 2*rs*(0.6*chairdim_e - yce(j));
      p(i,(N-1)/2.0+j) = p(i,(N-1)/2.0+j) + 2*rs*(0.6*chairdim_e - yce(j));
    end
end

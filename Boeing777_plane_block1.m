function [ p,numx,numy,xdim,ydim,x,y ]=Boeing777_plane_block1(rb,re1,re2,cb,ce,ab,ae,cwb,cwe,rwb,rwe,sd,cd,lrb,lre,ncb,nce,as,rs,cs,space_front,space_back,x_exit)
...
for i=(space_front+x_exit+1):(x_exit+space_front+2*rb*rwb)
    for j=1:NCb
...
%front exit path
for i=1:space_front+x_exit
    for j=1:(N/2.0)
      p(i,j) = p(i,j) + 3*rs*(ydim/2.0-y(j));
      p(i,N-j+1) = p(i,N-j+1) + 3*rs*(ydim/2.0-y(j));
    end
end
...
%     for j=1:N-ncb*cwb
%       p(i,j) = p(i,j) + 3*rs*(y(j)-ydim);
%     end
%     for j=1:ncb*cwb 
%       p(i,N-j+1) = p(i,N-j+1) + 3*rs*(ycb(j)-chairdim_b);
%     end
    for j=1:(N/2.0)
...
for i=(2*space_front+space_back+2*x_exit+2*rb*rwb+2*re1*rwe+1):(3*space_front+2*space_back+3*x_exit+2*rb*rwb+(2*re1+1)*rwe+lre)
    for j=1:(N/2.0)
      p(i,j) = p(i,j) + 3*rs*(y(j)-ydim/2.0);
      p(i,N-j+1) = p(i,N-j+1) + 3*rs*(y(j)-ydim/2.0);
    end

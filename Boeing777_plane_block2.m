function [ p,numx,numy,xdim,ydim,x,y ]=Boeing777_plane_block2(rb,re1,re2,cb,ce,ab,ae,cwb,cwe,rwb,rwe,sd,cd,lrb,lre,ncb,nce,as,rs,cs,space_front,space_back,x_exit)
...
%economy cabin1
for i=(2*space_front+space_back+2*x_exit+2*rb*rwb+1):(2*space_front+space_back+2*x_exit+2*rb*rwb+2*re1*rwe)
    for j=1:NCe
...
for i=(x_exit+space_front+2*rb*rwb+1):(2*space_front+space_back+2*x_exit+2*rb*rwb)
    for j=1:NCb
      p(i,j) = p(i,j) + rs*(chairdim_b - ycb(j));
      p(i,N-j+1) = p(i,N-j+1) + rs*(chairdim_b - ycb(j));
    end
